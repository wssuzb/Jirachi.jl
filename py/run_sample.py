...
sf_bin = 10 ** np.arange(0, 5.05, 0.05)
cv_bin = 10 ** np.arange(0, 5.1, 0.1)

lc1_dat = np.loadtxt("../../../archive_data/ngc4395/Montano22_night1/%s_4395.txt"%str(used_band[0]), usecols=[0, 1, 2])
lc2_dat = np.loadtxt("../../../archive_data/ngc4395/Montano22_night1/%s_4395.txt"%str(used_band[1]), usecols=[0, 1, 2])

t1, y1, e1 = lc1_dat[:, 0], lc1_dat[:, 1], lc1_dat[:, 2]
t1 = (t1 - t1[0]) * 24 * 3600 - 1e-9
# t1, y1, e1 = np.array(t1), np.array(y1), np.array(e1)
lc1 = var.lightcurve(t1, y1, e1)

t2, y2, e2 = lc2_dat[:, 0], lc2_dat[:, 1], lc2_dat[:, 2]
t2 = (t2 - t2[0]) * 24 * 3600 - 1e-9
# t2, y2, e2 = np.array(t2), np.array(y2), np.array(e2)
...
flux_ratio = [0.9] #f['flux_ratio'].value.flatten()
