...
f = h5py.File('./run_bin_103.68_sf_noise_cut_2_sf_bin_0.05_cv_bin_0.1_montano22_n1_nsim_1000_mode_both_nsigma_3_band_i_z_sun14.h5', 'r') 
...
#
# idx = np.isnan(bin_sf_1_sf)
# idx_t = np.where(bin_sf_1_t[~idx] <= t1_max, True, False)
# popt, pcov = curve_fit(fit_sf, bin_sf_1_t[~idx][idx_t], bin_sf_1_sf[~idx][idx_t], p0=[1, 1e3, 1, 0.005],
#                       sigma= bin_sf_1_sf_err[~idx][idx_t],
#                        bounds=([0, 0, 0, 1e-3], [10, t1_max, 2, 0.1]))
# print(t1_max)
...
axes['A'].plot(fit[1][0], fit[1][1], lw=0.6, color=colors[1], ls=(0, (5, 3)))#, label=r'$%s$-band'%str(band_pair[0]))
...
# ==========================================================================
# f = h5py.File('./run_result/run_bin_103.68_sf_bin_0.05_cv_bin_0.1_montano22_n1_nsim_1000_mode_both_nsigma_5_band_i_z_sun14.h5', 'r') 
# fit = f['fit']
# band1, band2 = f['band'].value.flatten()
...
# axes['C'].hist(num_cut, bins=use_bin, histtype='step',ls=(0, (1, 0.5)), lw=0.5, color='blue',  label=r'$3\sigma$ cut', zorder=4)
# axes['C'].hist(num_pos, bins=use_bin, histtype='step',lw=0.5, color='brown', ls='-.', label=r'$5\sigma$-${\rm cut}~\&~C_{f/m}>0$', zorder=3)

axes['C'].legend(loc='upper left', frameon=False, fontsize=4)


# axes['B'].scatter(cv_1_t[idx_cv_1], cv_1_y[idx_cv_1], facecolors='none',
#                   marker='*', s=15,  linewidth=0.2, color='m')#facecolors='none', 

# axes['B'].errorbar(cv_1_t[idx_cv_1], cv_1_y[idx_cv_1],
#                    xerr= np.vstack((bin_width_all[idx_cv_1], bin_width_all[idx_cv_1])), 
#                    yerr=np.vstack((cv_1_y_err[idx_cv_1], cv_1_y_err[idx_cv_1])), color='black',
#                    mew=0.2, elinewidth=0.2, lw=0.3,ls='-.', capsize=0.5)#, label='%s vs. %s'%(band_pair[0], 
                   
# axes['D'].scatter(cv_2_t[idx_cv_2], cv_2_y[idx_cv_2], facecolors='none',
#                   marker='*', s=15,  linewidth=0.2, color='blue')#facecolors='none', 
...
plt.savefig('./mag_flux_new_sun14.png', dpi=500, bbox_inches='tight')
# plt.savefig('./figure/fig_mag_flux_new_sun14_sf_0.1_cv_0.5.pdf',bbox_inches='tight', pad_inches=0.05)
# plt.show()